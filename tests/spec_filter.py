...
class SpecTestCase(BaseTestCase):
    ...
    def test_next(self):
        cursor1 = get_page(collection=self.collect, sort=[['a.b', 1]], limit=20)
        self.assertEqual([i['a']['b'] for i in cursor1], list(range(0, 20)))

        cursor2 = get_page(collection=self.collect, sort=[['a.b', 1]], limit=20, position=cursor1.paging.next_position)
        self.assertEqual([i['a']['b'] for i in cursor2], list(range(20, 40)))

        cursor1 = get_page(collection=self.collect, sort=[['a.b', 1]], limit=20, position=cursor2.paging.previous_position)
        self.assertEqual([i['a']['b'] for i in cursor1], list(range(0, 20)))
...
class SortTestCase(BaseTestCase):
    ...
    def test_asc_num(self):
        cursor = get_page(sort=[['a', 1]], limit=3, collection=self.collect)
    ...
    def test_asc_arg(self):
        cursor = get_page(sort=[['a', ASCENDING]], limit=3, collection=self.collect)
        self.assertEqual([i['a'] for i in cursor], [0, 1, 2])

    def test_desc_arg(self):
        cursor = get_page(sort=[['a', DESCENDING]], limit=3, collection=self.collect)
        self.assertEqual([i['a'] for i in cursor], [9, 8, 7])
...
class FilterTestCase(BaseTestCase):
    ...
    def test_single_filter(self):
        ...
        cursor1 = get_page(**params)
        self.assertEqual([i['a'] for i in cursor1], [0, 2])

        cursor2 = get_page(**params, position=cursor1.paging.next_position)
